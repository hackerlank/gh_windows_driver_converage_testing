...
#define DRIVER_NAME                       L"\\Device\\cvg"
#define DEVICE_NAME                       L"\\DosDevices\\cvg"
...
NTSTATUS WriteMemory(ULONG ulAddr, UCHAR* pData, ULONG ulLen)
{
	...
	ptrMdl = IoAllocateMdl(ulAddr, ulLen, FALSE, FALSE, NULL);
	if(ptrMdl == NULL){
...
NTSTATUS DispatchIoctl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
	...
	switch(code){
	case IOCTL_CVG_GET_MODULE_LIST:
		...
		break;
	case IOCTL_CVG_GET_BASE_ADDRESS:
		...
		*((ULONG*)pIrp->AssociatedIrp.SystemBuffer) = ptrBaseAddr;
		len = sizeof(ULONG);
		break;
	case IOCTL_CVG_GET_ENTRY_ADDRESS:
		...
		*((ULONG*)pIrp->AssociatedIrp.SystemBuffer) = ptrEntryAddr;
		len = sizeof(ULONG);
		break;
	case IOCTL_CVG_GET_SECTION_COUNT:
		...
		len = sizeof(ULONG);
		break;
	case IOCTL_CVG_GET_SECTION_ADDRESS:
...
#else
		RtlCopyMemory((UCHAR*)pIrp->AssociatedIrp.SystemBuffer, ptrBaseAddr, stack->Parameters.DeviceIoControl.InputBufferLength);
#endif
		len = stack->Parameters.DeviceIoControl.InputBufferLength;
		break;
	case IOCTL_CVG_WRITE_MEMORY:
		ptrBaseAddr = *(ULONG*)pIrp->AssociatedIrp.SystemBuffer;
	...
	case IOCTL_CVG_NEW_MEMORY:
		...
		*((ULONG*)pIrp->AssociatedIrp.SystemBuffer) = ptrEntryAddr;
		...
		len = sizeof(ULONG);
		break;
	case IOCTL_CVG_FREE_MEMORY:
		ptrEntryAddr = *((ULONG*)pIrp->AssociatedIrp.SystemBuffer);
		DbgPrint("Free memory: addr 0x%X\n", ptrEntryAddr);
		ExFreePool(ptrEntryAddr);
		break;
	case IOCTL_CVG_PATCH_MODULE:
		DbgPrint("Patch Kernel Module: %ws\n", (PWCHAR)pIrp->AssociatedIrp.SystemBuffer);
		bRet = KernelGetModuleBase3(pDevObj->DriverObject, (PWCHAR)pIrp->AssociatedIrp.SystemBuffer, &ptrBaseAddr, &ptrEntryAddr);
		DbgPrint("Base Address: 0x%X\n", ptrBaseAddr);
		DbgPrint("Entry Address: 0x%X\n", ptrEntryAddr);
		DbgPrint("Patched result: %d\n", bRet);
      
		DbgPrint("PE Header: %c%c\n", ((PUCHAR)ptrBaseAddr)[0], ((PUCHAR)ptrBaseAddr)[1]);
		if((((PUCHAR)ptrBaseAddr)[0] == 'M') && (((PUCHAR)ptrBaseAddr)[1] == 'Z')){
			IMAGE_DOS_HEADER *pDosHeader = (IMAGE_DOS_HEADER*)ptrBaseAddr;
			IMAGE_NT_HEADERS *pImageNTHeader = (IMAGE_NT_HEADERS*)((PUCHAR)ptrBaseAddr + pDosHeader->e_lfanew);
			IMAGE_FILE_HEADER *pFileheader = &pImageNTHeader->FileHeader;
			...
			for(i=0; i<pFileheader->NumberOfSections; i++){
				...
				if(strcmp(sectionHeader->Name, ".text") == 0){
					...
					ptrMdl = IoAllocateMdl((PVOID)(ptrBaseAddr + sectionHeader->VirtualAddress), sectionHeader->Misc.VirtualSize, FALSE, FALSE, NULL);
					if(ptrMdl == NULL){
						DbgPrint("IoAllocateMdl failed\n");
					}
					else{
						__try{
							...
							if(ptrBuffer == NULL){
								...
								if(status == STATUS_SUCCESS){
									DbgPrint("MmProtectMdlSystemAddress successed\n");
									DbgPrint("OPCode: 0x%X", *((ULONG*)ptrBuffer));
									*((ULONG*)ptrBuffer) = 0x88888888;
									DbgPrint("OPCode: 0x%X", *((ULONG*)ptrBuffer));
								}
								else{
									DbgPrint("MmProtectMdlSystemAddress failed 0x%X\n", status);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
	...
	//ListKernelModule(pDrvObj, NULL, 0);
	return STATUS_SUCCESS;
}
